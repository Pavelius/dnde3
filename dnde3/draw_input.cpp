...
static void render_info(const creature& e) {
	...
	int y1 = y;
	int x1 = x;
	y += fiela(x, y, tw, "СЛ", e.get(Strenght), e.getbase(Strenght));
	y += fiela(x, y, tw, "ИН", e.get(Intellegence), e.getbase(Intellegence));
	x += dx;
	y = y1;
	y += fiela(x, y, tw, "ЛВ", e.get(Dexterity), e.getbase(Dexterity));
	y += fiela(x, y, tw, "МД", e.get(Wisdow), e.getbase(Wisdow));
	x += dx;
	y = y1;
	y += fiela(x, y, tw, "ТЛ", e.get(Constitution), e.getbase(Constitution));
	y += fiela(x, y, tw, "ХР", e.get(Charisma), e.getbase(Charisma));
	x += dx + 6;
	y = y1;
	y += fiela(x, y, tw, "АТ", attm.attack, attm.attack, "%1i%%", attr.attack, attr.attack, "%1i%%");
	y += fiela(x, y, tw, "БР", e.get(Protection), e.get(Protection), "%1i%%", e.get(Armor), e.getbase(Armor));
	x += dx + 50;
	y = y1;
	...
	y += fiela(x, y, 52, "ПБ", e.get(Deflect), e.get(Deflect), "%1i%%");
	x = x1;
	y = y1 + draw::texth() * 2;
	// Draw encumbrance
...
static hotkey indoor_keys[] = {
	...
	{'U', "Использовать объект", &creature::useobject},
