...
static void weapon_skill_ability(stringbuilder& sb, manual& mn, answeri& an) {
	...
	if(mn.value.type == Skill) {
		...
		sb.adds("Отражает шанс нанести врагу критический удар.");
		//if(ei.weapon.attack)
...
BSDATA(manual) = {
	...
	{Skill, ProficiencyDaggers, 0, "Позволяет виртуозно атаковать кинжалом.", weapon_skill},
	{Skill, ProficiencyMaces, 0, "Позволяет наносить более опасные удары тупым оружием.", weapon_skill},
	{Skill, ProficiencyPolearms, 0, "Позволяет наносить более опасные колющие удары древковым оружием.", weapon_skill},
	{Skill, ProficiencyStaff, 0, "Позволяет крутить посох вокруг себя как настоящий виртуоз.", weapon_skill},
